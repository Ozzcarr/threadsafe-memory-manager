...
static void *(*myfn_mmap)(void *ptr, size_t length, int prot, int flags, int fd,
                          off_t offset);
...
static void init() {
    ...
    if (!myfn_malloc || !myfn_free || !myfn_calloc || !myfn_realloc ||
        !myfn_memalign || !myfn_mmap || !myfn_munmap) {
...
void *malloc(size_t size) {
    ...
    if (myfn_malloc == NULL) {
        if (!initializing) {
            ...
            fprintf(stdout,
                    "jcheck: allocated %lu bytes of temp memory in %lu chunks "
            ...
            if (tmppos + size < sizeof(tmpbuff)) {
                ...
                fprintf(stdout,
                        "jcheck: too much memory requested during "
...
void *memalign(size_t blocksize, size_t bytes) {
    ...
    char buffer[70];
    int len =
        sprintf(buffer, "rMEMALING (%ld, %ld) @ %p\n", blocksize, bytes, ptr);
    write(1, buffer, len);

    return ptr;
}

void *mmap(void *ptr, size_t length, int prot, int flags, int fd,
           off_t offset) {
    static int initializing = 0;
    if (myfn_mmap == NULL) {
        if (!initializing) {
