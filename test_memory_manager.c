...
typedef struct {
    ...
    bool simulate_work;     // Flag to simulate work in the thread, i.e. put the
...
// Function to calculate memory allocations for threads based on redistribution
// logic
size_t *calculate_thread_allocations(int num_threads, size_t total_memory) {
    ...
    // Calculate redistribution from the first half to the second half of the
    // threads
...
/*
    This is a generic test function that can be used to test any function from
   the single-threaded test cases in a multithreading context. The function
   takes a pointer to the test function, the number of threads to create, the
   size of the memory pool, and the name of the function being tested (used for
   printing purposes only).
*/

void testAcrossConfigurations(void (*test_func)(TestParams),
                              TestParams params) {
    ...
    // If mem_size is 0, we run with single memory size, ie. the function
    // requires a fixes size mem_size
    ...
    // Run the test function for all combinations of num_threads, mem_sizes, and
    // repetitions
...
void run_concurrent_test(void *(*test_func)(void *), TestParams params,
                         char *function_name) {
    printf_yellow("  Testing \"%s\" (threads: %d, mem_size: %zu) ---> ",
                  function_name, params.num_threads, params.memory_size);
    ...
    for (int i = 0; i < params.num_threads; i++) {
        ...
        int rc = pthread_create(&threads[i], NULL, (void *(*)(void *))test_func,
                                &params_t[i]);
...
void *test_alloc_and_free(void *arg) {
    ...
    memset(block1, data->thread_id,
           block1_size);  // Unique pattern using thread_id

    size_t block2_size =
        block1_size * 3;  // Corrected from undefined block2_size variable
    char *block2 = (char *)mem_alloc(block2_size);
    my_assert(block2 != NULL);
    memset(block2, data->thread_id + block1_size,
           block2_size);  // Another unique pattern offset by 100

    my_barrier_wait(
        &barrier);  // Assuming barrier is defined somewhere globally
...
/*
 * This function is used to test the allocation of random blocks of memory and
 * then freeing them in a multithreading context. The test passes if all
 * allocations and deallocations are successful.
 */
...
void *thread_alloc_free(void *arg) {
    ...
    for (int i = 0; i < data->num_blocks; i++) {
        ...
        my_assert(data->block_pointers[i] !=
                  NULL);  // Make sure the allocation was successful
    }
...
void test_random_blocks_multithread(TestParams params) {
    printf_yellow(
        "  Testing \"mem_alloc\" and mem_free for random blocks (threads: %d, "
        "max_block_size: %zu) ---> ",
    ...
    void *block_pointers[total_blocks];  // Array to hold pointers to allocated
                                         // blocks
    ...
    for (int i = 0; i < params.num_threads; i++) {
        ...
        thread_data[i].block_pointers =
            &block_pointers[i * thread_data[i].num_blocks];
    }
...
/*
 * This function is used to test the resizing of memory blocks in a
 * multithreading context. Each thread will allocate a block of memory, resize
 * it, and then free it. The test passes if all threads complete successfully.
 */
...
void test_resize_multithread(TestParams params) {
    printf_yellow("  Testing \"mem_resize\" (threads: %d) ---> ",
                  params.num_threads);

    pthread_t threads[params.num_threads];
    size_t initial_size = 100;  // Each thread starts with 100 bytes

    mem_init(1024 * params.num_threads);  // Initialize enough memory for all
                                          // threads to work comfortably

    // Launch threads to perform the resize operation
    for (int i = 0; i < params.num_threads; i++) {
        if (pthread_create(&threads[i], NULL, thread_resize,
                           (void *)initial_size) != 0) {
...
void test_exceed_single_allocation_multithread(TestParams params) {
    printf_yellow(
        "  Testing \"allocation exceeding pool size\" (threads: %d) ---> ",
        params.num_threads);

    pthread_t threads[params.num_threads];
    size_t size_to_allocate = 2048;  // Each thread will try to allocate 2KB

    mem_init(1024);  // Initialize with 1KB of memory, intentionally less than
                     // required per thread

    // Create threads that will each try to allocate more memory than available
    for (int i = 0; i < params.num_threads; i++) {
        if (pthread_create(&threads[i], NULL, alloc_exceeding_memory,
                           (void *)size_to_allocate) != 0) {
            perror("Failed to create thread");
            exit(EXIT_FAILURE);
        }
    }
    ...
    if (fail_count == 0) {
        ...
        printf_red(
            "[FAIL]: Some threads incorrectly succeeded in allocation.\n");
    }
}
...
void *cumulative_alloc(void *arg) {
    ...
    for (int i = 0; i < 128; i++) {
        ...
        if (blocks[i] == NULL) {
            // if (debug)
            printf_yellow(
                "    Allocation failed as expected for size %zu in thread %d\n",
    ...
    my_barrier_wait(&barrier);
    for (int i = 0; i < 128; i++) mem_free(blocks[i]);
    free(blocks);
...
void test_exceed_cumulative_allocation_multithread(TestParams params) {
    printf_yellow(
        "  Testing \"cumulative allocations exceeding pool size\" (threads: "
        "%d, mem_size: %zu) ---> \n",
        params.num_threads, params.memory_size);

    size_t *sizes = calculate_thread_allocations(
        params.num_threads, params.memory_size + params.num_threads / 2);
    ...
    for (int i = 0; i < params.num_threads; i++) {
        ...
        if (pthread_create(&threads[i], NULL, cumulative_alloc,
                           &thread_data[i])) {
            perror("Failed to create thread");
            exit(EXIT_FAILURE);
        }
    ...
    for (int i = 0; i < params.num_threads; i++) {
        pthread_join(threads[i], &status);
        if ((long)status == 1) {  // Expected failure
            pass_count++;
        }
    }
    ...
    if (pass_count >= 1) {  // At least one thread failed to allocate beyond the
                            // limit as expected
        printf_green(
            "[PASS]: At least one thread failed to allocate beyond the limit "
            "as expected.\n");
    } else {
...
/*
 * This function is used to test the allocation of memory beyond the total
 * available memory pool. The test passes if all allocations fail as expected.
 */
...
void *thread_overcommit_alloc(void *arg) {
    ...
    if (debug)
        printf_yellow(
            "    Thread %d successfully allocated %zu bytes initially\n",
            data->thread_id, data->block_size);
    ...
    if (extra_block != NULL) {
        if (debug)
            printf_red(
                "    Thread %d unexpectedly succeeded in allocating extra "
                "memory\n",
                data->thread_id);
        mem_free(
            extra_block);  // Cleanup if allocation was unexpectedly successful
        my_barrier_wait(&barrier);
    ...
    if (debug)
        printf_yellow(
            "    Thread %d correctly failed to allocate extra memory as "
            "expected\n",
            data->thread_id);
    mem_free(initial_block);  // Clean up initial block
...
void test_memory_overcommit_multithread(TestParams params) {
    printf_yellow(
        "  Testing \"memory overcommitment\" (threads: %d, mem_size: %zu) "
        "---> ",
        params.num_threads, params.memory_size);
    ...
    size_t memory_per_thread =
        params.memory_size /
        params.num_threads;  // Each thread tries to allocate 1KB
    mem_init(
        params.memory_size);  // Initialize with 1KB of memory, intentionally
                              // less than required per thread

    // Setup thread parameters and create threads
    for (int i = 0; i < params.num_threads; i++) {
        params_t[i].thread_id = i;
        params_t[i].block_size = memory_per_thread;
        if (pthread_create(&threads[i], NULL, thread_overcommit_alloc,
                           (void *)&params_t[i]) != 0) {
            perror("Failed to create thread");
            exit(EXIT_FAILURE);
        }
    ...
    for (int i = 0; i < params.num_threads; i++) {
        pthread_join(threads[i], &status);
        if ((long)status == 1) {  // Check for expected failures
            failures++;
        }
    }
    ...
    if (failures == 0) {
        ...
        printf_red(
            "[FAIL]: Some threads unexpectedly succeeded in allocating memory "
            "beyond the limit.\n");
    }
}
...
void *thread_repeated_fit_reuse(void *arg) {
    ...
    for (int i = 0; i < iterations; i++) {
        ...
        if (block == NULL) {
            if (debug)
                printf_red(
                    "    Thread %ld failed to allocate block of %zu bytes on "
                    "iteration %d\n",
...
void test_repeated_fit_reuse_multithread(TestParams params) {
    ...
    printf_yellow(
        "  Testing \"repeated exact fit reuse\" (num_threads: %d, memory_size: "
        "%zu, repeat: %d) ---> ",
    ...
    thread_data_t params_t[params.num_threads];
    size_t block_size =
        params.memory_size / params.num_threads;  // Size of each memory block

    mem_init(params.memory_size);  // Initialize with 1KB of memory, enough for
                                   // all threads if they reuse properly
    ...
    for (int i = 0; i < params.num_threads; i++) {
        if (pthread_create(&threads[i], NULL, thread_repeated_fit_reuse,
                           &params_t[i]) != 0) {
            perror("Failed to create thread");
            exit(EXIT_FAILURE);
        }
    ...
    if (failures == 0) {
        ...
        printf_red(
            "[FAIL]: Some threads failed to consistently reuse blocks.\n");
    }
}

void *repeated_allocate_and_free(void *arg) {
    thread_data_t *data = (thread_data_t *)arg;
    int cycles = data->iterations;  // Use iterations from thread data for
                                    // number of cycles

    for (int i = 0; i < cycles; i++) {
        void *block =
            mem_alloc(data->block_size);  // Allocate using the block_size from
                                          // thread data
        if (block == NULL) {
            if (debug)
        ...
        if (debug)
            printf_yellow(
                "    Thread %d allocated and will now free %zu bytes\n",
                data->thread_id, data->block_size);
...
void *repeated_allocate_in_fragment(void *arg) {
    ...
    for (int i = 0; i < cycles; i++) {
        ...
        if (block != NULL) {
            if (debug)
                printf_yellow(
                    "    Thread %d allocated %zu bytes in fragmented memory\n",
                    data->thread_id, data->block_size);
            mem_free(block);
        } else {
            if (debug)
                printf_red(
                    "    Thread %d failed to allocate %zu bytes in fragmented "
                    "memory\n",
                    data->thread_id, data->block_size);
        }
...
void test_memory_fragmentation_multithread(TestParams params) {
    printf_yellow(
        "  Testing \"memory fragmentation handling\" (threads: %d, mem_size: "
        "%zu, iterations: %d) ---> ",
        params.num_threads, params.memory_size, params.iterations);
    mem_init(params.memory_size);  // Initialize with specified memory size to
                                   // accommodate load
    ...
    // Dynamically calculate block size based on available memory and the number
    // of threads
    size_t base_block_size =
        params.memory_size /
        (params.num_threads *
         3);  // Adjust factor if necessary to prevent over-allocation

    // Setup thread parameters
    for (int i = 0; i < params.num_threads; i++) {
        params_t[i].thread_id = i;
        params_t[i].block_size =
            base_block_size *
            (i % 3 +
             1);  // Multiplicative factor to vary block size: 1x, 2x, 3x
        params_t[i].iterations =
            params.iterations;  // Set the number of allocation-free cycles

        if (i % 2 == 0) {
            // Even-indexed threads create fragmentation
            pthread_create(&threads[i], NULL, repeated_allocate_and_free,
                           &params_t[i]);
        } else {
            // Odd-indexed threads attempt to fill fragmented spaces
            pthread_create(&threads[i], NULL, repeated_allocate_in_fragment,
                           &params_t[i]);
        }
    }
...
void *thread_function(void *arg) {
    ...
    for (int i = 0; i < num_allocations; i++) {
        ...
        // blocks[i], thread_id * num_allocations + i); Write a unique pattern
        // based on thread_id and index i
        memset(blocks[i], thread_id * num_allocations + i, block_size);

        // Optionally, simulate some work or delay
        if (params->simulate_work) usleep(rand() % 1000);
    }

    // my_barrier_wait(&barrier);

    for (int i = 0; i < num_allocations; i++) {
        sanityCheck(block_size, blocks[i],
                    (char)(thread_id * num_allocations + i));
...
void run_concurrency_test(TestParams params) {
    printf_yellow(
        "  Running concurrency test with %d threads, %d allocations per "
        "thread, and block size %zu bytes --> ",
        params.num_threads, params.num_blocks / params.num_threads,
        params.block_size);
    ...
    mem_init(params.num_blocks *
             params.block_size);  // Initialize with enough memory for the test
    ...
    long micros =
        ((seconds * 1000000) + end_time.tv_usec) - (start_time.tv_usec);
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 2) {
        ...
        printf(
            "  1. tests various functions across variious configurations "
            "(number of threads, memory sizes,  iterations)\n");
        printf(
            "  2. stress tests various functions with various configurations. "
            "This may take some time (especially if simulate_work flag is set "
            "to true.\n");
        printf(
            "  3. test_looking_for_out_of_bounds, needs "
    ...
    size_t blockSize;
    bool simulate_work =
        false;  // set this to true to see the benefits of multithreading
    ...
    switch (atoi(argv[1])) {
        ...
        case 0:
            ...
            printf(
                "\n*** Testing various functions with a base number of "
                "threads: ***\n");
            run_concurrent_test(test_alloc_and_free,
                                (TestParams){.num_threads = base_num_threads,
                                             .memory_size = 1024},
                                "mem_alloc and mem_free");
            run_concurrent_test(test_zero_alloc_and_free,
                                (TestParams){.num_threads = base_num_threads,
                                             .memory_size = 1024},
                                "zero alloc and free");

            test_resize_multithread(
                (TestParams){.num_threads = base_num_threads});

            test_exceed_single_allocation_multithread(
                (TestParams){.num_threads = base_num_threads});
            test_exceed_cumulative_allocation_multithread((TestParams){
                .num_threads = base_num_threads,
                .memory_size = 1024});  // TODO: Fix this to be able to run with
                                        // various configurations

            test_memory_overcommit_multithread((TestParams){
                .num_threads = base_num_threads, .memory_size = 1024});

            for (int i = 0; i < 4; i++)
                test_repeated_fit_reuse_multithread(
                    (TestParams){.num_threads = base_num_threads,
                                 .memory_size = 1024,
                                 .iterations = pow(10, i)});

            test_memory_fragmentation_multithread((TestParams){
                .num_threads = base_num_threads, .memory_size = 2048});
            test_random_blocks_multithread((TestParams){
                .num_threads = base_num_threads, .block_size = 1024});

            break;

        case 1:
            printf(
                "\n*** Testing various functions across variious "
                "configurations (number of threads, memory sizes,  "
                "iterations): ***\n");
            testAcrossConfigurations(test_resize_multithread,
                                     (TestParams){.memory_size = 1024});
            testAcrossConfigurations(test_exceed_single_allocation_multithread,
                                     (TestParams){.memory_size = 1024});

            for (int i = 1; i < 6; i++)
                test_exceed_cumulative_allocation_multithread((TestParams){
                    .num_threads = pow(2, i), .memory_size = pow(2, 11 + i)});
            break;
            testAcrossConfigurations(test_memory_overcommit_multithread,
                                     (TestParams){.memory_size = 1024});
            testAcrossConfigurations(test_repeated_fit_reuse_multithread,
                                     (TestParams){.iterations = 1});
            testAcrossConfigurations(test_memory_fragmentation_multithread,
                                     (TestParams){.iterations = 1});
            testAcrossConfigurations(
                test_random_blocks_multithread,
                (TestParams){.memory_size = 1024, .block_size = 1024});
        ...
        case 2:
            ...
            for (int i = 1; i < 4; i++) {
                for (int j = 1; j < 5; j++) {
                    run_concurrent_test(
                        test_alloc_and_free,
                        (TestParams){.num_threads = i,
                                     .memory_size = pow(2, 9 + j)},
                        "mem_alloc and mem_free");
                }
            }
            ...
            for (int i = 2; i < 6; i++) {
                test_random_blocks_multithread(
                    (TestParams){.num_threads = pow(2, i), .block_size = 1024});
            }
            ...
            for (int i = 0; i < 9; i++)
                run_concurrency_test(
                    (TestParams){.num_threads = pow(2, i),
                                 .num_blocks = allocs,
