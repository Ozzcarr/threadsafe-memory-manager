...
void test_list_insert_multithread(TestParams *params) {
    printf_yellow("  Testing list_insert (threads: %d, nodes: %d) ---> ",
                  params->num_threads, params->num_nodes);
    ...
    thread_data_t *thread_data =
        malloc(params->num_threads * sizeof(thread_data_t));
    ...
    for (int i = 0; i < params->num_threads; i++) {
        ...
        if (pthread_create(&threads[i], NULL, thread_insert_function,
                           &thread_data[i])) {
    ...
    // Verify and clean up
    // Note: Verification can be complex in multithreaded contexts due to node
    // order variations
    printf_green("[PASS].\n");
    list_cleanup(&head);
...
void test_list_insert_after_multithread(TestParams *params) {
    printf_yellow("  Testing list_insert_after (threads: %d, nodes: %d) ---> ",
                  params->num_threads, params->num_nodes);

    Node *head = NULL;
    list_init(&head, sizeof(Node) *
                         (params->num_nodes + 1));  // +1 for the initial node
    ...
    for (int i = 0; i < params->num_threads; i++) {
        ...
        thread_data[i].start_value = i * nodes_per_thread;
        thread_data[i].num_nodes = nodes_per_thread;
        if (pthread_create(&threads[i], NULL, thread_insert_after_function,
                           &thread_data[i])) {
            perror("Failed to create thread");
        }
    }
    ...
    // Verify node count
    my_assert(list_count_nodes(&head) ==
              params->num_nodes + 1);  // +1 for the initial node
...
void *thread_insert_before(void *arg) {
    ...
    for (int i = 0; i < data->num_nodes; i++) {
        // Data calculation could be adjusted to fit specific patterns or test
        // cases
        uint16_t insert_data =
            (data->thread_id + 1) * 100 + i;  // Example pattern
...
void test_list_insert_before_multithreaded(TestParams *params) {
    printf_yellow(
        "  Testing list_insert_before with %d threads, each inserting %d nodes "
        "---> ",
        params->num_threads, params->num_nodes);
    Node *head = NULL;
    list_init(&head,
    ...
    for (int i = 0; i < params->num_threads; i++) {
        thread_data[i].head = &head;
        thread_data[i].prev_node =
            nodes[i];  // Each thread starts at a different initial node
        thread_data[i].num_nodes =
            params->num_nodes / params->num_threads;  // Distribute nodes evenly
        pthread_create(&threads[i], NULL, thread_insert_before,
                       &thread_data[i]);
    }
    ...
    int expected_count = params->num_threads + params->num_nodes +
                         1;  // Initial nodes + inserted nodes + head
...
void *thread_delete_function(void *arg) {
    ...
    for (int i = 0; i < data->num_nodes; i++) {
        ...
        uint16_t data_value =
            data->thread_id * data->num_nodes +
            i;  // Adjust the multiplier if necessary to match insertion pattern
...
void test_list_delete_multithreaded(TestParams *params) {
    printf_yellow("  Testing list_delete with %d threads, nodes: %d ---> ",
                  params->num_threads, params->num_nodes);
    Node *head = NULL;
    ...
    for (int i = 0; i < params->num_threads; i++) {
        ...
        pthread_create(&threads[i], NULL, thread_delete_function,
                       &thread_data[i]);
    }
    ...
    my_assert(list_count_nodes(&head) ==
              0);  // Assuming all nodes are supposed to be deleted
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 2) {
        ...
        printf(
            "Basic Operations with a base number of threads (4) and nodes "
            "(1024):\n");
        printf(
            " 1. test_list_insert - Test basic list insert operations with a "
            "base number of threads\n");
        printf(
            " 2. test_list_insert_after - Test list insert after a given "
            "node\n");
        printf(
            " 3. test_list_insert_before - Test list insert before a given "
            "node\n");
        printf(" 4. test_list_delete - Test delete operation\n");

        printf(
            "\nStress testing basic operations with various numbers of threads "
            "and nodes:\n");
        printf(" 5. test_list - Test multiple configurations\n");
        printf(
            " 6. test_list_insert_after - Test multiple insertions after a "
            "given node\n");
        printf(
            " 7. test_list_insert_after - Test multiple insertions after a "
            "given node\n");
